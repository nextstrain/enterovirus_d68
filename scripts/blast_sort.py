...
if __name__ == '__main__':
    ...
    parser.add_argument('--check_duplicates', action='store_true', help="whether to check duplicates, default true")
    parser.add_argument('--no_hit_seqs', help="file containing list of sequences that had no blast hit")
    parser.add_argument('--no-check_duplicates', dest='check_duplicates', action='store_false')
    parser.add_argument('--not_matched_seqs', help="file containing a list of sequences not meeting minimum match length")
    ...
    #read in sequences, store names
    fasta_sequences = SeqIO.parse(open(args.seqs), 'fasta')
    seq_names = [fasta.id for fasta in fasta_sequences]
    fasta_sequences = SeqIO.parse(open(args.seqs), 'fasta')
    ...
    no_keep_blast = set(list(blast.loc[blast.send - blast.sstart < matchLen, "qseqid"]))
    ...
    blasted_ids = set(blast.qseqid)
    no_vp_match = [s for s in seq_names if s not in blasted_ids]
    ...
    if args.no_hit_seqs:
        pd.DataFrame(no_vp_match).to_csv(args.no_hit_seqs, sep='\t', index=False, header=False)

    if args.not_matched_seqs:
        pd.DataFrame(no_keep_blast).to_csv(args.not_matched_seqs, sep='\t', index=False, header=False)
    ...
    sepr = ","
    if re.search('.tsv$', args.meta):
        sepr = "\t"
    
    meta = pd.read_csv(args.meta, sep=sepr, index_col=False)
    ...
    unique_dup_hits = set(meta_matchLen[dups]['strain'])
    if sum(dups) != 0:
        print("\nRemoving {} two-match-hits. Note that two-match 'matchlength's will be only for the FIRST match.".format(sum(dups)))
        print(f"A unique list of the {len(unique_dup_hits)} strains is: {unique_dup_hits}")
    ...
    meta_matchLen.to_csv(args.out_meta, sep=sepr, index=False)
